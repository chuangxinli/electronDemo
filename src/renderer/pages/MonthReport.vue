        ...
        <el-dialog
            ...
            <div>
                ...
                <ul class="taskTable" v-if="curDownTask.taskId">
                    ...
                    <li v-if="curDownTask.gradeReportInfo
&& curDownTask.gradeReportInfo.gradeReportList.length > 0">
                        <div class="div_table">
                            <div class="div_tr">
                                <div class="div_td">
                                    <span>年级报告</span>
                    ...
                    <li v-if="curDownTask.classReportInfo
&& curDownTask.classReportInfo.classReportList.length > 0">
                        <div class="div_table">
                            <div class="div_tr">
                                <div class="div_td">
                                    <span>班级报告</span>
                    ...
                    <li v-show="curDownTask.classPersonReportList && curDownTask.classPersonReportList.length > 0 && curDownTask.classPersonReportList[0].children && curDownTask.classPersonReportList[0].children.length > 0"
                        v-for="item,index in curDownTask.classPersonReportList[0].children">
                        <div class="div_table">
                            <div class="div_tr">
                                <div class="div_td">
                                    <span>个人报告</span>
                ...
                if(this.global.downTaskList.length > 0){
                    for(let i = 0, len = this.global.downTaskList.length; i < len; i++){
                        if((row.homeworkId == this.global.downTaskList[i].taskId) && this.global.downTaskList[i].reportType == this.reportType){
                            if(!this.global.downTaskList[i].gradeReportInfo.isComplete){
                                this.$message({
                                    type: 'info',
                            ...
                            if(!this.global.downTaskList[i].classReportInfo.isComplete){
                                this.$message({
                                    type: 'info',
                                    message: '当前考试任务正在下载，不能重复下载，请等待本次任务下载完成，再接着下载！',
                                    showClose: true
                            ...
                            if(!this.global.downTaskList[i].classPersonReportList[0].isComplete){
                                this.$message({
                                    type: 'info',
                                    message: '当前考试任务正在下载，不能重复下载，请等待本次任务下载完成，再接着下载！',
                                    showClose: true
                ...
                if(this.global.downTaskList.length > 0){
                    for(let i = 0, len = this.global.downTaskList.length; i < len; i++){
                        if((this.taskId == this.global.downTaskList[i].taskId) && this.global.downTaskList[i].reportType == this.reportType){
                            this.global.downTaskList.splice(i, 0)
                            break;
                        }
                    }
                }
