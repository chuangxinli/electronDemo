...
<script>
  export default {
    ...
    computed: {
      page() {
        ...
        if(this.totalPage < 8){
          ...
          return arr
        }else if(this.currentPage > 4 && this.totalPage - this.currentPage < 4){
          return [
            {type: 'num', page: 1},
            {type: 'pre5', page: '···'},
            {type: 'num', page: this.totalPage - 5},
            {type: 'num', page: this.totalPage - 4},
            {type: 'num', page: this.totalPage - 3},
            {type: 'num', page: this.totalPage - 2},
            {type: 'num', page: this.totalPage - 1},
            {type: 'num', page: this.totalPage},
          ]
        }else if(this.currentPage < 5 && this.totalPage - this.currentPage > 3){
          return [
            {type: 'num', page: 1},
            {type: 'num', page: 2},
            {type: 'num', page: 3},
        ...
        }else if(this.currentPage > 4 && this.totalPage - this.currentPage > 3){
          return [
            {type: 'num', page: 1},
            {type: 'pre5',page: '···'},
            {type: 'num', page: this.currentPage - 2},
            {type: 'num', page: this.currentPage - 1},
            {type: 'num', page: this.currentPage},
            {type: 'num', page: Number(this.currentPage) + 1},
            {type: 'num', page: Number(this.currentPage) + 2},
            {type: 'next5', page: '···'},
            {type: 'num', page: this.totalPage},
          ]
        }
    ...
    methods: {
      ...
      changePage(item){
        if(item.type === 'pre5'){
          this.emitParent(this.currentPage - 5 > 0 ? this.currentPage - 5 : 1, this.pageSize)
          ...
          this.emitParent(Number(this.currentPage) + 5 <= this.totalPage ? Number(this.currentPage) + 5 : this.totalPage, this.pageSize)
        }
      },
      confirmPageSize(){
