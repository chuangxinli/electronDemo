...
function dealJson(res, docxArr) {
    let jsonArr = []
    for (let i = 0, len = docxArr.length; i < len; i++) {
        let jsonObj = {}
        jsonObj.question = []
        let primaryStr = ''
        let curItemType = 'title'
        let itemTypeNum = 0
        let hasSubItem = false  //题组题
        let curItemProperty = ''
        mammoth.convertToHtml({path: docxArr[i]}, {
            ...
            for (let i = 0, len = $(document.body).children().length; i < len; i++) {
                ...
                if ($(document.body).children()[i].tagName.toLowerCase() == 'p' || $(document.body).children()[i].tagName.toLowerCase() == 'table') {
                    ...
                    if (/^\s*#\s*#/.test(primaryStr)) {
                        ...
                        if (dealSpace('适用学期（册）').test(primaryStr)) {
                            jsonObj.Term = getTitleInfo('适用学期（册）', primaryStr)
                            continue
                        }
            ...
            if(jsonArr.length === docxArr.length){
                res.send({jsonArr})
            }
            /*fs.writeFile(`./json/temp_${i}.json`, JSON.stringify(jsonObj), (err) => {
                if (err) {
                    console.log(err)
                    return
                }
            ...
            fs.writeFile(`./html/temp_${i}.html`, temp, (err) => {
                if (err) {
                    console.log(err)
                    return
                }
...
module.exports.dealJson = dealJson
